...
**Aug 6, 2026 realignment — the "Wpbl" Google Tasks list.** A new Google Tasks list ("Wpbl", pulled via `npm run tasks`) captured 7 hands-on **polish / bugfix / hygiene** items that weren't on the feature-forward Next list above. Sequenced into three sprints ahead of the marquee predictions feature (#9), which is untouched:
  - **Sprint A — fix & refine shipped surfaces** *(in progress):*
    - ✅ **Tracking data missing for a previous game** — root-caused: the Aug 5 BOS@LA game has **zero TrackMan in the feed itself** (Aug 1/2 have full 200 rows each), so it's a league-side feed gap. Behind it, a real defect: `mode=active` never re-fetches a game once stored `final`, so late-arriving TrackMan (which routinely posts *after* a game goes Final) would be lost forever. Fixes: (a) **ingest backfill** — ✅ **deployed:** `wpbl-ingest` re-fetches recently-final games still missing tracking (widened to 21 days) so late data self-heals, and now sources tracking from the **uncapped `/activity` endpoint** (removing the old 200-row/game cap); (b) **client empty-state** — the Pitch Data tab now shows on any played game with an explicit "No pitch tracking" note instead of silently vanishing (ships with the build).
...
**Aug 9, 2026 realignment — v1.31.0 → v1.32.0 shipped (Sprint A closed; pitch-location maps + the tracking-source fix).**
- ✅ **Sprint A closed** — box-score mobile column reorder, the Pitch Data game-highlights strip (hardest pitch · hardest hit · first hit), and the Pitch Data "No pitch tracking" empty-state all shipped (v1.31.0). The **tracking backfill is deployed** (`wpbl-ingest`): it force re-fetches recently-final games (21-day window) and now sources tracking from the **uncapped `/activity` endpoint**, so late-arriving TrackMan self-heals — and this **lifts the 200-row/game cap** that had forced the coverage caveats and parked the "game duration" idea. Warmup pitches are excluded from velocity attribution, and the feed's 1-hour-early game times are corrected on ingest.
- ✅ **Pitch-location maps** (`PitchLocation.tsx`, v1.32.0) — a WPBL pitcher's page now maps every tracked pitch over the plate on a pitch-type-colored strike zone, plus a per-pitch-type mini map with count + avg velocity. Delivers the "player/team pitch-velo" depth from item #13 via the richest available cut.
- ✅ **New-tracking Home banner + scoreboard snap** (v1.32.0) — the WPBL home page flags a fresh batch of pitch tracking and links to the Tracking tab (closes item #8's open "small Home teaser card linking to tracking"); the home scoreboard strip now opens snapped to the most recent/upcoming games instead of the oldest final.
- ✅ **Also shipped:** first grand slam added to the Hall of Firsts (v1.31.0); shareable WPBL player links (v1.32.0); the tracking coverage disclaimer reworded to lead with the partial-data caveat and auto-derive the tracked-game count.
- 🔬 **Next (unchanged priority, now the clear front):** #9 **game predictions / pick'em (+ bots)** — the marquee open engagement item — then #10 daily-standouts Home card, #11 league primer + stat glossary, #12 Game-Center win probability. Sprint C (Settings accent-color picker) still pending.

**Aug 9, 2026 — default section flipped to WPBL.** The bare domain (`sportydolphin.fun` with no `/mlb` or `/wpbl`) now redirects to **`/wpbl`** instead of `/mlb` (root redirect in `App.tsx`), and the installed PWA renamed from "MLB Picks" to **"sportydolphin"** (`apple-mobile-web-app-title` + manifest `name`/`short_name`) with its `start_url` pointed at `/wpbl`. MLB stays one tap away via the MLB | WPBL toggle.

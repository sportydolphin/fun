...
- 🟡 **Phase 1 underway.** Sequence: **1a** entry modal → 1b box-score read view → 1c player pages + season totals → 1d paste-parser (deferred until a real box score exists post-Aug 1).
  - ✅ **1a — Box-score entry modal (admin-only)** — `src/wpbl/GameEntry.tsx`: per-game modal to set status/score/innings + per-team batting (AB/R/H/2B/3B/HR/RBI/BB/SO/SB/HBP/CS) and pitching (IP-as-outs/H/R/ER/BB/SO/HR/BF/P/decision) lines; wide tables scroll horizontally. Reached from an admin-only "Enter/Edit" chip on each Schedule row (`WpblApp` takes `isAdmin` from `App.tsx`). Save = `saveWpblGameResult` (update game + delete-then-insert lines; owner RLS gates it). Standings/scores already recompute from finals. Verified in preview (modal opens, rosters load, add-row works, no console errors) via a dev-sim admin session. **⚠️ The actual Supabase write is NOT yet verified** — needs a real signed-in owner (dev-sim token can't pass RLS); test by entering a game while signed in.
  - ✅ **1b — Box-score read view** — `src/wpbl/GameDetail.tsx`: public game-detail modal (tap any Schedule row, or Home "Next up"). Header shows final score with winner emphasized (or date/time for unplayed); per-team batting (AB/R/H/HR/RBI/BB/SO/SB + totals row) and pitching (IP/H/R/ER/BB/SO/HR + W/L/S/H decision) tables, horizontally scrollable. Empty states for "not entered yet" (final) vs "not played yet" (scheduled). Admin gets an "Edit result" shortcut into the 1a modal. Row-click vs the admin Enter chip disambiguated with stopPropagation. Verified in preview (unplayed-game modal renders, no errors); **populated box score unverified until real lines exist** (needs a live owner save first).
- ⬜ **Next:** owner does a real signed-in save (verifies both 1a write + 1b populated box score); then **1c** (player pages + season totals aggregated from lines). Still to supply real team colors + hosted logos. Follow-ups: browser back-stack for team/game detail; global footer still says "Not affiliated with MLB" on the WPBL route.

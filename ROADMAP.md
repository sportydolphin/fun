...
- **Phase 3 (hardening + depth + postseason)** 🔬🎮 **← next** — see "Next" below for the prioritized list. Done: ingest hardening, loose ends, team pages, search integration. Now (realigned Aug 4, live-season-driven): ship the ingest deploy → live-data QA sweep → richer standings → playoffs (delayed, still date-driven for mid-Sept), then (later) porting the fun MLB mechanics. "Followed players/teams + notifications" was dropped (tiny league — low value).
...
- 🔬 **Next (Phase 3 — hardening → depth → engagement → postseason), in priority order:**
  ...
  *(Realigned Aug 4, 2026 — search shipped; "followed players/teams + notifications" dropped from the active plan; the live season now drives priorities. New order below.)*
  5. ⚠️ **Close out the ingest deploy** — the server-side dedup + ingest-health indicator (item 1) are **built but not shipped**: needs `supabase functions deploy wpbl-ingest` + a one-off re-ingest to clear the already-stored phantom and light the admin freshness dot. **Highest priority** because it's finished code affecting *live* data — until it ships, a phantom "Not Started" game can sit beside the real final, and the health indicator reads dark. (User action; no code left to write.)
  ...
  7. **Richer standings** — the Standings page shows only W / L / run-diff. Add win% (PCT), games back (GB), and current streak (W2 / L3); optionally last-10 and home/away splits, plus a head-to-head tiebreak in `computeStandings` (today ties break on run-diff only). It's a daily-check page mid-season and currently the thinnest view.
  8. **Playoffs & championship view** — *delayed but still on the clock:* the inaugural season ends mid-Sept with a postseason, so a bracket / clinch tracker needs to land before then. Slotted after the hardening + standings work.
  9. **Later — port the fun MLB mechanics** — streak report cards, milestone watch, and predictions/pick'em + bots for WPBL games (the live feed makes this viable).
  - ⏸️ **Deferred — Followed teams/players + notifications.** Dropped from the active plan: with four teams and ~118 players the whole league fits on two screens, so per-entity follows/alerts add little over just opening the section. Revisit only if the league grows or users ask.
  - 💡 **Minor / opportunistic** — WPBL recents in the empty-query search dropdown (search shipped without them); stat glossary / "how it works" blurb; any polish surfaced by the QA sweep.

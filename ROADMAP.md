...
- 🟡 **Predictions 2.0, finish** 🎮 — Wilson-ranked board + running record shipped (v1.9.0). **v1.19.0:** streak badges + heater banner (current/best streak persisted to `prediction_stats`, 🔥 badge on the board, banner on the Predictor card and My Stats) and a smart rival — the 🧠 Sabermetric Bot in `run-bots.mjs` (Pythagorean expectation from runs scored/allowed, resolved by log5 with a home edge). **Still open: weekly/monthly leaderboard cuts** — needs a nightly precompute (`update-prediction-boards.mjs` → `prediction_boards`), since correctness isn't stored and can't be aggregated in SQL.
...
**Suggested next three:** Predictions 2.0 weekly/monthly leaderboard cuts (the one remaining piece — build `update-prediction-boards.mjs` + `prediction_boards`, reusing the streak-precompute template, then a time-window toggle on the board) → close-game push alerts (reuses the shipped push plumbing + the milestone-alert pattern) → a second Phase 2 daily game (Daily Trivia is the offseason-proof pick). Phase 1 is fully shipped (streak boards, trade deadline HQ, playoff odds, Milestone Watch); Predictions 2.0 now has streak badges/heater + a smart rival bot (v1.19.0), leaving only the windowed cuts.

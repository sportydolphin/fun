...
- Where the season's MISSING TrackMan data is, and every public route to it that has been tried → [`docs/TRACKMAN.md`](docs/TRACKMAN.md), re-checkable with [`scripts/probe-wpbl-tracking.mjs`](scripts/probe-wpbl-tracking.mjs). We hold 2 of 25 finals and, as of Sep 5, 2026, no other public site holds any: the rival tracking board renders empty, so the gate caught everyone. `wpbl-tracking-watch` answers "did our mirror grow"; this answers "is there another door", which is the question that was being re-asked from memory every few weeks

...
flowchart TB
    ...
    subgraph EXT["🌐 External services"]
        ...
        substack["Substack: towards a more perfect game<br/>+ The Rising Fastball<br/>archive API + RSS<br/>(blocks GitHub Actions, not Supabase)"]
        discord["Discord<br/>webhooks: board · box scores · highlights<br/>bot: /player · /score · /predict interactions"]
...
flowchart LR
    subgraph Main["Main sections (lazy chunks)"]
        ...
        wreading["/wpbl/reading<br/>ReadingPage.tsx + Reading.tsx"]
    ...
    wtabs --> wstats["Stats sub-boards:<br/>Players · Teams · Bests · Find · Pitch by pitch · Run value ·<br/>Tracked (hidden until radar returns) · Draft"]
    wpbl --> wplayers
...
flowchart TB
    subgraph WPBL["WPBL: mirror of the official feed (written by wpbl-ingest)"]
        ...
        t_beats["cron_heartbeats<br/>(generic per-job health)"]
        t_alert["admin_alert_state<br/>(health-alert dedupe)"]
        ...
        t_bsky["wpbl_bluesky_recap_posts + wpbl_bluesky_start_posts<br/>(finals and game-start reminders posted to Bluesky; no edit, so no hash)"]
...
| `wpbl-youtube-sync` | `0,30 14-23,0-3` | `sync-wpbl-youtube`, `post-wpbl-discord-highlights` | Mirror WPBL YouTube uploads → `wpbl_videos` (the Highlights section of the season recap + each game's reel), then post anything new to the Discord highlights channel in the same pass. **Two streams:** game highlight reels, identified from the title by `classify()`, and **Shorts**, identified by probing `youtube.com/shorts/<id>` (200 = Short, 303 to `/watch` = not) because no title reveals a Short. Each stream seeds itself the first time the job meets it, so adding one never floods the channel |
...
| `wpbl-tracking-listen` | `20 18` + `20 21` (Sep-Oct) | `listen-wpbl-tracking` | Hold the league's LIVE SOCKET open through a game and write any TrackMan that arrives → `wpbl_pitch_tracking`. **The only path left to tracking**: `/v1/games/{id}/activity` has answered `401 missing api key` since Sep 1, 2026 and the boxscore no longer carries `tracking_activity` as a key at all, but `wss://stats.womensprobaseballleague.com/v1/ws?channel=boxscore:<id>` needs no key and the league's own client handles a `tracking_activity_updated` envelope on it. Writes tracking and nothing else; box scores stay the ingest's. Exits in seconds when no game is inside the window, and says in as many words whether anything was published |
| `wpbl-site-calendar` | `0 */3` | `sync-wpbl-site-calendar` | Mirror the league's own WEBSITE calendar (`wp-json/wpbl/v1/calendar-events`, the one behind womensprobaseballleague.com/schedule/) → `wpbl_site_games`. **A different publication from the stats feed**, and the only source for a fixture that feed has not carried yet: it needs two clubs on a row, so it held nothing for the postseason while the website had all eleven games with times, tickets and, for the semifinals, **which club bats last**. Upserts only, never deletes, so a short read costs nothing |
| `wpbl-recaps-sync` | `0 1-7,13` | `recaps-sync` | Mirror This is Women's Baseball's game recaps as LINKS → `wpbl_recaps` (the recap card on a finished game), resolving each headline to the game it is about. Hourly through the window their posts actually land in, since they file within an hour of the final and a once-a-day job would leave the game page bare for the evening a reader is most likely to be on it; the 13:00 run catches the one filed the following night. Upserts only ([`docs/RECAPS.md`](docs/RECAPS.md)). Writes a `cron_heartbeats` row (failure-only, like the game-start sender) |
...
| `wpbl-bluesky-recaps` | `repository_dispatch` (`wpbl-final`), + `*/15` (Mar-Oct) as backstop | `post-wpbl-bluesky-recaps` | Post a finished WPBL game to our own Bluesky timeline: the recap as text, the box score as a rendered image (Bluesky has no monospace, so the Discord table cannot be reused). Bluesky posts cannot be EDITED, so unlike the Discord recap nothing is ever re-sent: a game is published only once the LEAGUE's own `source_updated_at` on it is 45 minutes old, so corrections land first. That basis matters: it used to be measured from when this job first saw the game, which needed two runs to publish anything and, against GitHub's real cadence, made a 45-minute window a 5-to-12-hour one. The database now triggers it (`wpbl-bluesky-nudge`, pg_cron table below). Never backfills |
| `wpbl-bluesky-game-start` | `repository_dispatch` (`wpbl-bluesky-game-start`), + `*/15` (Mar-Oct) as backstop | `post-wpbl-bluesky-game-start` | Post a "starting soon" reminder for a WPBL game to our own Bluesky timeline: text only, no card. The pre-game twin of the recap poster, with the window pointing the opposite way: it publishes only while a game is still `scheduled` and its first pitch is inside a short window ahead, and never once it has started, because a permanent "first pitch soon" over a game already underway cannot be edited away. Carries the league's own wall-clock start (via `applyLeagueStartTimes`), not a countdown, since the post outlives the minute it was written; adds the round and series line in the postseason. No seed and no backfill: only games about to start are ever eligible. The database triggers it (`wpbl-bluesky-start-nudge`, pg_cron table below) |
| `wpbl-mention-watch` | `*/15` (Mar-Oct) | `watch-wpbl-mentions` | Search Reddit posts, Reddit comments (search does not index them, and the question is usually a reply in somebody else's game thread) and Bluesky for people asking where to follow a WPBL game, and digest the threads worth answering into a private Discord channel. **Finds threads, never replies to them**: the only place it posts is our own webhook. Facebook is absent because no permitted automated path to group content exists (see [`docs/DISCORD.md`](docs/DISCORD.md)) |
...
| `wpbl-roster-fork-check` | `15 8,20` | `check-roster-forks` | Fail when one club carries two players with the same SURNAME, which is almost always one player the ingest has split in two. Same loud-failure policy as the row above and the same reason for it: across the 118 players there are six shared surnames and every pair is split across two clubs, so the expected output is nothing and a red X IS the signal. Written the day the league published Claire O'Sullivan as **Catherine** in a postseason box score under a freshly minted feed id: the `(club, name)` fallback matched nobody, a second player row was inserted, her game went under it, and a READER caught it. **The fix is never a looser name matcher** (`matchFeedName` already takes a prefix or one edit per half of a name, and Catherine/Claire are four apart; matching on surname alone would silently weld two real teammates into one person). It prints how many games each row has, when each was created, and whether the two have ever shared a box score, which a split player never can; a person then runs `wpbl_merge_players(keep, dupe)` or adds the pair to `KNOWN_PAIRS` |
...
| Job | Schedule | Action |
|---|---|---|
| `wpbl-ingest-active` | `*/2 * * * *` | `pg_net` POST → `wpbl-ingest` `{mode:"active"}` (key from Vault), **only when `wpbl_ingest_due()`**: every tick within `[-2, +3]` Central days of a game (feed or site calendar) or while a game reads live, otherwise the :00 tick of 00/06/12/18 UTC. The floor keeps reading `/games` all winter, so a newly published season reopens the two-minute cadence with nobody touching anything; `admin-health-alert` relaxes its ingest staleness window to 7h on the same test |
...
| `wpbl-bluesky-start-nudge` | `*/5 * * * *` | `wpbl_nudge_bluesky_game_start()` → `{event_type:"wpbl-bluesky-game-start"}`, when a scheduled game's first pitch (site-calendar start over the feed's) is between 12 min ago and 40 min away and it has not been posted. Coarse on purpose; the poster re-checks with the full start-time correction and is the only decider. A distinct event type from the push reminder's `wpbl-game-start`, so the two fire independently. **A late reminder is a wrong one**, so the window errs wide: a missed nudge falls to the schedule backstop, a wrongly-timed permanent post cannot be taken back |


**All five of these nudges share `wpbl_dispatch_workflow(event, key, gap)`**, which holds the Vault
...
**And the one way this fails silently**: `repository_dispatch` only starts a workflow whose file on
the DEFAULT BRANCH declares the matching type, so a dispatch answers `204` and runs nothing at all
until the trigger is merged. Verified with a hand call the moment the migration landed: `204`, no
run, because the triggers were still on a branch.
...
| **WPBL live socket** (`wss://stats.womensprobaseballleague.com/v1/ws`) | `listen-wpbl-tracking` | The league's own live channel, `?channel=boxscore:<gameId>`, unauthenticated. Three envelope types: `boxscore_snapshot`, `boxscore_updated` and **`tracking_activity_updated`**. We read only the last, since the ingest owns box scores. **This exists because the REST tracking route is gone**: `/activity` is 401 and the boxscore's `tracking_activity` key is absent (retested Sep 7, 2026 with `?include=`, `?tracking=1`, `?expand=`, `?with=` — byte-identical). Found by reading their site's `wpbl-draft-picks/assets/js/live-stats.js`. The same socket would also replace the ingest's two-minute polling, which is not done |
| **WPBL website calendar** (`womensprobaseballleague.com/wp-json/wpbl/v1/calendar-events`) | `sync-wpbl-site-calendar` | The league's marketing site, which publishes a schedule the stats feed does not: a game row there needs two clubs, so the feed carried no postseason at all while this had all eleven games. Mirrored into `wpbl_site_games` for **who bats last** (the six semifinal games, and the championship's once the finalists are known), first-pitch times, the league's own game pages and ticket links. **Not a second opinion on anything the feed carries**: scores, box scores, plays and standings stay the feed's, and only `postseasonScheduleRows` reads this, only for a fixture the feed has no row for. Which series a row belongs to is parsed from its slug (`semi-final-series-a-playoff-game-2`), the only field that says; an unrecognised slug stores nulls and the app falls back to its own published constant |
...
| **The Rising Fastball** (`therisingfastball.substack.com`) | `sync-wpbl-substack` | D.A. Espinoza's women's baseball coverage, **mirrored with explicit permission (granted Sep 2026)** on the same terms as the row below: headline, dek, cover, link, never the body. Only the league posts: a title starting "WPBL" or a profile naming a rostered player, English editions only (the Japanese and Spanish translations are dropped). `source = 'rising-fastball'` |
| **towards a more perfect game** (`towardsamoreperfectgame.substack.com`) | `sync-wpbl-substack` | An independent writer's WPBL coverage, **mirrored with her explicit permission (granted Aug 17, 2026)**; her name is on every card and every card links straight to her post: headline, dek, cover, word count and link → `wpbl_articles`, matched to players/clubs/games. **The article body is never stored.** The RSS feed carries the full text, and the job reads it only to find names in it; `wpbl_articles` has no body column so the rule is enforced by the schema. Every surface links out to her site |
| **This is Women's Baseball** (`thisiswomensbaseball.com/f.rss`) | `sync-wpbl-recaps` | An independently run WPBL outlet that files one recap per game, **linked with their explicit permission (granted Sep 12, 2026)**; their site is All Rights Reserved, so that permission is the whole basis for this and their name is on every card. **Links only**: the headline, their URL, their cover image's URL and the date go into `wpbl_recaps`, which has no column for prose and never will. Their feed offers the lede and the sync does not read it, because on a 180-word recap the lede is roughly half the article. The cover is EMBEDDED from their CDN, never mirrored, and resized by their own isteam parameters (248 KB to 18 KB). Placing a headline on one of our games is the hard part and lives in `derive/recaps.ts`: two phases, six rules, 33 of 33 games matched with nothing ambiguous, and an unmatched game shows no card rather than a guess. The feed is a 50-item window that ignores every paging parameter, so the sync upserts and never deletes ([`docs/RECAPS.md`](docs/RECAPS.md)) |
| **RetroWPBL** (`github.com/exu6jh/RetroWPBL`) | `sync-wpbl-retro` | An independent hand transcription of the season into Retrosheet format, **used with the transcriber's explicit permission (granted Aug 21, 2026)**; the repository carries no licence file, so that permission is the whole basis for using it and is credited in the UI wherever the data renders. Only the event files' `info` records are read: first pitch, `timeofgame`, the umpiring crew and the weather, none of which exist anywhere in the league feed. **The play records are deliberately not mirrored** (we hold the play-by-play in more depth already, and a second copy would be a second truth); the value of that independent transcription is as a CHECK on ours, which is a different job. The one exception, added Sep 9, 2026, is a play the league published EMPTY: `wpbl-play-gap-fill` writes those into `wpbl_play_corrections`, on the reasoning that a row with no batter, no event and no narrative is not a competing account of the play but the absence of one, so there is no second truth to create. Attendance is present upstream and is 0 on every row, so it is not stored. **Umpire names come from `biodata/biofile.csv`, not `umpires/UMPIRES2026.txt`**, which is stale; an id that resolves to no name is dropped and counted, never rendered |
...
| `wpbl-substack-sync` | pg_cron (hourly, :17) + manual | Mirror both writers' WPBL coverage into `wpbl_articles`, one pass per publication (`SOURCES` in `derive/articles.ts`): headline, dek, cover, word and clip counts, and which players/clubs/game each post is about. Read by the `/wpbl/reading` page, Home's latest-post line, the game story card and the player/team "written about" lists. Never stores her article text. Runs here rather than in GitHub Actions because Substack serves Cloudflare's JS challenge to Actions runners and not to Supabase ([`docs/READING.md`](docs/READING.md)). Logic shared with `npm run substack-sync` via [`src/wpbl/substackSync.ts`](src/wpbl/substackSync.ts) |
...
| **Feedback email** (the `email_new_feedback` trigger on `feedback`, via `pg_net` to Resend) | `resend_api_key`, `feedback_email_to` (support@sportydolphin.fun). Sent from feedback@sportydolphin.fun, verified in Resend. A no-op until both exist, and never able to refuse a note. See [`20260925001112_email_feedback_to_the_owner.sql`](scripts/migrations/20260925001112_email_feedback_to_the_owner.sql) | Supabase **Vault** |
...
- What a situation is worth, and what each play was worth (the Run value board on Stats, live for everyone since v1.52.0, Aug 27, 2026) → [`src/wpbl/derive/runExpectancy.ts`](src/wpbl/derive/runExpectancy.ts), drawn by [`src/wpbl/RunValueView.tsx`](src/wpbl/RunValueView.tsx). The table is built from THIS league's plays, never a borrowed major-league one: the WPBL scores ~15 runs a game over seven innings, so every state is worth roughly double. ONE board draws it, all of it, since Sep 10, 2026 (v1.80.0): the leaderboard, the count grid, the take/swing split, the per-event prices and the steal card. The last two came off a board of their own called Findings, whose cards live in [`src/wpbl/PlayValue.tsx`](src/wpbl/PlayValue.tsx) with the traffic that argued for the merge (readers swept that row rather than choosing it, and 41% of the people who read Run value never reached the prices it is built on). **The explanation lives here and only here**: one shut card, three steps (a situation is worth something, with the 24-cell grid as its evidence; a play is worth what it changed, with the formula as three named terms; one real play, with those same terms as a ledger), then the caveats. The play-value card carries a row that opens it and scrolls to it rather than a second copy. The example is CHOSEN by `workedExample()` (the play nearest its own event's average) rather than written down, because the table moves with every ingest and pasted numbers would silently stop matching it. The ledger's total is the sum of its own rounded terms rather than the rounded true value, so the column a reader is invited to check actually adds up
- The standings plotted against the calendar (the "Standings by date" chart under the standings table) → [`src/wpbl/derive/seasonShape.ts`](src/wpbl/derive/seasonShape.ts), drawn by [`src/wpbl/SeasonShapeCard.tsx`](src/wpbl/SeasonShapeCard.tsx). Games above .500 per club, one column per playing date. **Pointing at it drives the standings table above**, which re-renders at that date, animates the clubs into their new places ([`src/wpbl/rowFlip.ts`](src/wpbl/rowFlip.ts), a FLIP: the DOM reorders so a screen reader's order stays true, and the movement is put back with transforms; the divider lines are a separate layer drawn OVER the rows from `useRowDividers`, because a border on a `<tr>` travels with the club and, under `border-collapse: collapse`, is painted beneath the opaque backing a moving row needs, so a reorder made every line in the table vanish for the length of the animation), and says which date it is showing; hover on a desktop, press and hold on a phone, arrow keys from the keyboard, or press play to walk the season. The gesture is `useChartScrub` in [`src/wpbl/chartScrub.ts`](src/wpbl/chartScrub.ts), lifted out of the win probability chart when this became the second caller: it tells a press apart from a scroll, and it sets `data-swipe-lock`, without which a drag across the chart pages to the Stats tab, since `/wpbl` swipes between its five pages on exactly that gesture. **It does not compute standings, it calls `computeStandings` once per column** on the games played up to that column: 20 dates over 34 games is nothing, and what it buys is that the chart cannot disagree with the table above it, tiebreaks included. The first cut accumulated its own wins and losses and was wrong inside an hour, breaking an 8-7 tie on games played where the table breaks it on head-to-head. **`standingsFinals` in [`season.ts`](src/wpbl/season.ts) is now the one definition of "a counted result, in the order it was played"**, shared with `computeStandings`; re-implementing it is how `headToHead` drew SF 6-0 over Boston above a 10-5 table. Two written findings sit under the chart: `leadStory` (days each club spent top of the table, and how often the lead changed hands), which is what a single standings frame structurally cannot report, and `biggestRun`, which **refuses a climb measured from the opening column** because every club starts level, so "up 4 games since opening day" is that club's standings row read back to it
...
- Any question the boards do not already answer (the Find board on Stats) → [`src/wpbl/derive/finder.ts`](src/wpbl/derive/finder.ts), drawn by [`src/wpbl/FindView.tsx`](src/wpbl/FindView.tsx). A search over every stored box-score line: up to six `stat op value` conditions, all ANDed, plus club, opponent, home/away and the season slice. It returns TWO answers, the matching games and a tally of who matched most, because the second is the more interesting one and is free once the matching is done. **Tractable only because of the scale**: Stathead exists because MLB has millions of game lines and needs an index; this league's whole season is ~750, already cached app-wide, so the engine is a filter and a sort with no RPC and no new table. Three things to keep. **Every condition is AND** and that is a stopping point rather than an oversight: "five strikeouts and no walks" is a question people ask and the OR of it is not, and a boolean tree would cost the whole UI. **An unrecognised field matches NOTHING rather than being ignored**, because a dropped condition answers a wider question than the URL asked and a reader cannot see that it happened. And **innings are the one field whose typed unit differs from its stored one**: "4.2" is four and two thirds, parsed with `ipToOuts` rather than Number, which is why the value control is a text input and not `type="number"`. The query encodes into ONE query param (`q=so.gte.5~bb.lte.1`), readable and URL-safe unencoded, so a shared link looks like a question rather than a tracking string; `STATS_URL_PARAMS` in StatsView is what carries it past WpblApp's mount stamp
- The best single GAME anybody had (the Bests board on Stats) → [`src/wpbl/derive/bests.ts`](src/wpbl/derive/bests.ts), drawn by [`src/wpbl/BestsView.tsx`](src/wpbl/BestsView.tsx), whose rows are [`GameLineRow.tsx`](src/wpbl/GameLineRow.tsx) shared with Find. Six hitting boards and four pitching ones, each ranking individual box-score lines rather than season totals: this is the league's record book, and it is the half of ROADMAP-WPBL #2 ("single-game records") that was buildable from what the mirror already holds. **It costs no fetch**: `fetchWpblAllLines` is cached app-wide and every line carries its own `game_id`, so the board is a sort over arrays the Stats tab already had. Four things worth keeping. **No qualifier**, because a counting stat over one game is not an estimate of anything and the sample IS the record; the day a rate board is added it needs a bar, and the bar belongs in `stats.ts` with the others. **Ties share a rank and the cut extends through one**, since a four-team league ties constantly (the strikeout record is held jointly) and publishing "the top five" while dropping somebody who did exactly what the fifth did is the one error a records board cannot make. **Finals only**, so a half-finished line cannot stand as a record; the cost is that a game whose status goes backwards drops off until it settles, which is visible and self-correcting. And **the club comes off the LINE, never the roster**, which is the section's standing identity trap. **There is deliberately no game score**: Bill James's formula spends 50 points before an out is recorded, and this league's longest outing is 5.1 IP, so it ranks a two-inning relief cameo (59) one point behind the season's best start (61). "Longest scoreless outing" is the part of that question a box score can settle without inventing a constant; the honest composite would price an outing against `derive/runExpectancy.ts`, which needs the play log this module does not fetch
...
- Where a hitter puts the ball, out of a feed that publishes no hit location at all → [`src/wpbl/derive/spray.ts`](src/wpbl/derive/spray.ts), drawn by [`src/wpbl/SprayChart.tsx`](src/wpbl/SprayChart.tsx) on the player page. There is no coordinate anywhere in the league's data, so the direction is READ OUT OF THE SCORER'S ENGLISH in each play's narrative ("singled to left field", "grounded out p to ss to 1b", "doubled down the 3b line"), which places 99% of the season's 1,535 batted balls into eleven zones. **Zones, never coordinates, and the card says so**: scattering dots inside a wedge would invent precision the league never recorded, and when RetroWPBL's hit locations arrive the same zones gain real points inside them. Three traps are pinned in `spray.test.ts` because each one was wrong first: everything after the first semicolon is a RUNNER ("advanced to second" is not a batted ball), a fielding relay is a chain whose FIRST link is the direction, and "down the 3b line" is a ball into the left-field corner rather than one the third baseman fielded. A switch hitter gets a chart and no pull rate, since the feed records handedness only on the roster
- What a count is worth, off the only thing the feed records about every pitch → [`src/wpbl/derive/countValue.ts`](src/wpbl/derive/countValue.ts), drawn by [`src/wpbl/CountBoard.tsx`](src/wpbl/CountBoard.tsx) on the Run value board. `pitch_sequence` is on 2,222 plate appearances and priced against the run-expectancy table built from these same plays, so it is in THIS league's runs (a half-inning here is worth roughly double a major-league one). Two traps. **The stored `balls` / `strikes` columns include the deciding pitch**, so a walk is recorded 4-0 and a strikeout 1-3, counts the batter never stood in and 633 of 2,222 rows carry: the sequence is walked and clamped instead, the same answer as the `live_state` trap in CLAUDE.md. And **`P` is the ball being put in play while the feed's own `pitch_events` calls it a pitchout** on all 1,563 of them, which would make this a league where a fifth of pitches are pitchouts; read the code, never the label. Measured: 0-0 is -0.01, 3-0 is +0.26, and the first pitch is worth 0.13 runs either way
- Which slice of the season a board is showing (the Regular season / Playoffs / Both switch on Stats) → `scopedLines` and `SeasonScope` in [`src/wpbl/season.ts`](src/wpbl/season.ts), passed as an optional last argument that DEFAULTS TO `regular` on every aggregate, so the OG share cards, the Discord `/player` card and the player pages publish exactly what they did before the switch existed. **The two slices fail in opposite directions and both are correct**: `regular` drops a game only on positive evidence (a feed rename costs a few games, never a blank standings page, as `countsInStandings` explains at length), while `postseason` KEEPS a game only on positive evidence, because "everything that does not look regular" is a definition of *unrecognised* and on that same rename it would republish all 30 regular-season games under a heading reading Playoffs. An empty playoff board is visibly broken and gets fixed; a full one made of the wrong games is invisible. `seasonScope.test.ts` pins both directions and the partition between them. The qualifier denominator moves with the scope too, or a hitter would need a full season's plate appearances to make a rate board in a three-game series
...
- Why a 46px face does not decode a 512px file → `wpblPortraitSet` in [`src/wpbl/portraits.ts`](src/wpbl/portraits.ts), which hands `PlayerPortrait` a `srcset` of a 128 thumb and the 512, and lets the browser choose. The thumbs are generated by [`scripts/make-wpbl-portrait-thumbs.py`](scripts/make-wpbl-portrait-thumbs.py) into `portraits/thumbs/` and `managers/thumbs/`; **the 512s stay**, because they are the source for the share cards and the player page at 2x actually wants one. A browser decodes at natural size, so thirty ballot tiles were holding ~30MB of bitmap for pictures of a face. `portraitAlpha.test.ts` fails on a portrait with no thumb
- Why every roster headshot has a transparent background → [`scripts/cut-out-wpbl-portraits.py`](scripts/cut-out-wpbl-portraits.py), keying the white studio paper out of [`src/wpbl/portraits/`](src/wpbl/portraits/) IN PLACE (the file is its own source; git is the only copy of what went in). `PlayerPortrait` fills its circle with the club colour and draws the photo over it, so a headshot that keeps its white renders perfectly and is the one face on the page wearing no colour. 65 of 118 were like that until Sep 10, 2026. **Run it when a new portrait lands**; `portraitAlpha.test.ts` fails on any bundled file with no alpha channel
- When a game starts, and which league source says so → `applyLeagueStartTimes` in [`src/wpbl/startTimes.ts`](src/wpbl/startTimes.ts). The stored `start_time` is a CENTRAL wall clock everywhere (one hub venue), converted to the reader's own zone at render by `formatGameTime`; nothing in the section prints a raw one. **The league's own calendar outranks the stats feed for a game still scheduled**, because the feed publishes a first pitch and then stops touching the row: on Sep 10, 2026 it had that night's semifinal an hour early for three days while `wpbl_site_games`, the league's published schedule and `POSTSEASON_SCHEDULE` all agreed on 6:00 PM. Applied in `WpblApp` (a second table the section must not wait on), and repeated in the bell and the push sender against their own reads
...
- What a game about to start looks like on a public timeline, and why it carries a wall clock rather than a countdown → [`src/wpbl/derive/blueskyGameStart.ts`](src/wpbl/derive/blueskyGameStart.ts) and [`scripts/post-wpbl-bluesky-game-start.ts`](scripts/post-wpbl-bluesky-game-start.ts)
...
- Replaying a finished game as a live one, for local work on any of the above → [`src/wpbl/dev/devLiveGame.ts`](src/wpbl/dev/devLiveGame.ts), driven from the dev gear. **It reaches the app through a slot in [`api.ts`](src/wpbl/api.ts) (`installWpblReadOverlay`) that `DevSettings` fills in, never an import the other way**: the simulator is reachable only from `DevSettings`, which is absent from the production bundle, so everything behind it goes too. An import from the read path into a dev module would have to survive on tree-shaking, and `mlb/dev/devSim.ts` is in the shipped bundle today for exactly that reason. The install itself is wrapped in `if (import.meta.env.DEV)`, because a bare call at module scope is a side effect that makes the file unremovable. Everything it publishes is derived from the plays the league actually logged; the box score is NOT replayed (one cumulative row per player, nothing to rewind to) and the panel says so
- Local-dev-only controls (skin, device sim, simulated login, notification tester, the WPBL Discord-invite undo, the live-game simulator above) → [`src/dev/DevSettings.tsx`](src/dev/DevSettings.tsx), gated on `import.meta.env.DEV`. **Anything it reaches into has to be import-light**: App.tsx imports that file eagerly while both sections are `lazy()`, so a convenience import from inside `MlbStats`/`WpblApp` pulls that section's chunk into the main bundle for every visitor, in production, to serve a control that only exists in dev. That is why the invite's dismissal key and its undo live in their own [`src/wpbl/discordInvite.ts`](src/wpbl/discordInvite.ts) rather than in `Home.tsx`

...
- **Android app (planned, not shipped):** a Trusted Web Activity generated by Bubblewrap
  from the live `manifest.webmanifest`, so the web deploy *is* the app deploy and the `.aab`
  is rebuilt only when the name, icon, package id or target SDK change. Blocked on a signing
  key: the URL bar only disappears once `/.well-known/assetlinks.json` carries both the
  upload and Play App Signing fingerprints. Plan of record, including why it is not
  Capacitor (Google blocks OAuth in embedded WebViews, and sign-in is a Google OAuth flow):

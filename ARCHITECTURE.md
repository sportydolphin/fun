...
| `wpbl-substack-sync` | `0 12-23` (hourly) | `sync-wpbl-substack` | Mirror two independent writers' WPBL posts → `wpbl_articles` (one row per post, `source` names the publication) (the `/wpbl/reading` page, Home's latest-post line, game story card, player/team "written about"), resolving each to the players, clubs and game it is about |

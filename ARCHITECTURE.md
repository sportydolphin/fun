...
**`voter_key` holds three kinds of value, on purpose.** The awards ballot keys on the browser's
...
cleared cache nor a second device; and a vote cast by reacting in the Discord keys on
`discord:<user id>` (`wpbl-discord-awards`). The third one is why the same person can answer
twice, once in each place, which is deliberate: the Discord vote is a bonus for being there, and
nothing joins a browser to a Discord account anyway. Nothing in the table tells the two apart and nothing needs
...
| `wpbl-discord-awards` | `5,20,35,50 * * 9 *` | `discord-awards` | The fan awards ballot as a reaction poll in Discord: one message per question, four numbered names each, and every run reads the reactions and casts them into `wpbl_award_votes` under `discord:<user id>` through `wpbl_cast_award_vote`, the same writer the browser uses. Votes in the Discord and on the site add up together, deliberately. One reaction is a vote, two is an unanswered question (Discord cannot refuse the second, so the arithmetic does), none withdraws it. The emoji-to-candidate map is stored in `wpbl_award_discord_polls` at post time, which is safe because the shortlists are seeded off a regular season that is over. Needs `DISCORD_AWARDS_CHANNEL_ID`; exits quietly without it |

...
| **Feedback email** (the `email_new_feedback` trigger on `feedback`, via `pg_net` to Resend) | `resend_api_key`, `feedback_email_to`. A no-op until both exist, and never able to refuse a note. See [`20260925001112_email_feedback_to_the_owner.sql`](scripts/migrations/20260925001112_email_feedback_to_the_owner.sql) | Supabase **Vault** |

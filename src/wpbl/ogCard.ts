...
export function wpblPlayerCard(
  ...
  return {
    ...
    description: describeSeason(player, teamName, batting, pitching),
...
function describeSeason(
  ...
  if (pitcherFirst) {
    const t = sumPitching(pitching as WpblPitchingLine[])
  ...
  if (hasBatting) {
    const t = sumBatting(batted as WpblBattingLine[])

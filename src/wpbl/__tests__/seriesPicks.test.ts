...
describe('when the FINAL can still be picked', () => {
  ...
  //   Semifinal B game 1: Sep 10, 7:30 PM Central = 2026-09-11T00:30:00Z (rain, moved off 6:00)
  ...
  it('is shut once ANY semifinal has started, for everybody', () => {
    ...
    expect(championshipPickOpen(bracket(), Date.parse('2026-09-11T00:31:00Z'))).toBe(false)
  })
  ...
  it("locks semifinal B at its published first pitch, not the feed's", () => {
    ...
    expect(seriesPickOpen(B, Date.parse('2026-09-11T00:29:00Z'))).toBe(true)
    expect(seriesPickOpen(B, Date.parse('2026-09-11T00:31:00Z'))).toBe(false)
  })

...
import {
  buildRunExpectancy, eventValues, fmtRunValue, playRunValues, stealEconomy, topRunners,
 ...
 */
function PlayValueCard({ rows, accent }: { rows: EventValue[]; accent: string }) {
  ...
  return (
    <SectionCard title="What every kind of play is worth">
      <Typography sx={{ fontSize: '0.9rem', lineHeight: 1.6, color: 'text.secondary' }}>
        Every play leaves a team a little better or worse off, and the amounts are not the ones
        you would guess from watching other leagues.
...
export default function WpblFindingsView({ games, battingLines, onOpenPlayer }: {
  ...
  return (
    <Box sx={{ ...READ_WIDTH, display: 'flex', flexDirection: 'column', gap: { xs: 1.5, sm: 2 } }}>
      ...
      <PlayValueCard rows={rows} accent={accent} />
      {experiments && (
